...
class merchant extends ecjia_merchant {
	...
	public function init() {
		...
		$this->assign('action_link', array('text' => __('添加新文章'), 'href' => RC_Uri::url('article/merchant/add')));
	...
	public function add() {
		...
		$this->assign('action_link', array('text' => __('文章列表'), 'href' => RC_Uri::url('article/merchant/init')));
	...
	public function insert() {
		...
		if ($article_type == 'article' || $article_type == 'related') {
			if (empty($content)) {
				return $this->showmessage(__('请输入文章内容'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		} 
		
		if ($article_type == 'redirect') {
			if (empty($link) || $link == 'http://' || $link == 'https://') {
				return $this->showmessage(__('请输入链接'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		} 
		
		if ($article_type == 'download' || $article_type == 'related') {
			if (empty($file_name)) {
				return $this->showmessage(__('请上传文件'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		}
		...
		$links[] = array('text' => __('返回文章列表'), 'href'=> RC_Uri::url('article/merchant/init'));
		$links[] = array('text' => __('继续添加新文章'), 'href'=> RC_Uri::url('article/merchant/add'));
		return $this->showmessage(__('文章已经添加成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('article/merchant/edit', array('id' => $article_id))));
	...
	public function edit() {
		...
		if (empty($article)) {
			$links = array('links' => array(array('text' => __('返回文章列表'), 'href' => RC_Uri::url('article/merchant/init'))));
			return $this->showmessage(__('没有找到该文章'), ecjia::MSGTYPE_HTML | ecjia::MSGSTAT_ERROR, $links);
		}
	...
	public function update() {
		...
		$article = get_merchant_article_info($id);
		if (empty($article)) {
			return $this->showmessage(__('没有找到该文章'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		if ($query) {
			...
			$note = sprintf(__('文章 %s 成功编辑'), stripslashes($title));
	...
	public function preview() {
		...
		$this->assign('action_linkedit', array('text' => __('编辑文章'), 'href' => RC_Uri::url('article/merchant/edit', array('id' => $id))));
		$this->assign('action_link', array('text' => __('文章列表'), 'href' => RC_Uri::url('article/merchant/init')));
	...
	public function link_goods() {
		...
		$this->assign('action_link', array('href' => RC_Uri::url('article/merchant/init'), 'text' => __('文章列表')));
	...
	public function insert_link_goods() {
		...
		ecjia_merchant::admin_log(__('关联商品').'，'.__('文章标题是').$article['title'], 'setup', 'article');
		return $this->showmessage(__('操作成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/merchant/link_goods', array('id' => $article_id))));
	}
	...
	public function remove_comment() {
		...
		if ($type == 'batch') {
			...
			if (!empty($ids)) {
				...
				if ($delete) {
					...
					foreach ($ids as $v) {
						...
						//记录日志
						ecjia_merchant::admin_log('评论内容为：'.$comment_info['content'].'，'.__('文章标题是'). $article['title'], 'batch_remove', 'article_comment');
					}
			...
			if (empty($comment_info)) {
				return $this->showmessage(__('没有找到该评论'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
			...
			if ($delete) {
				...
				//记录日志
				ecjia_merchant::admin_log(__('文章标题是'). $article['title'], 'remove', 'article_comment');
				return $this->showmessage(__('删除成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
			}
		}
		return $this->showmessage(__('操作失败'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
	...
	public function drop_file() {
		...
		return $this->showmessage(__('删除附件成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
	}
			...
			switch ($action) {
				...
				case 'move_to' :
					...
					foreach ($info as $v) {
						...
						ecjia_merchant::admin_log(__('转移文章'). $v['title'] . __('至分类') . $cat_name, 'batch_setup', 'article');
					}
					return $this->showmessage(__('批量转移操作成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/merchant/init')));
					break;
				default :
					break;
			}
	...
	public function article_comment() {
		...
		ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('文章评论列表')));
	...
	public function comment_check() {
		...
		if ($update) {
			...
			return $this->showmessage(__('操作成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $pjax_url));
		}
	}
	...
	public function batch_check() {
		...
		if (!empty($ids)) {
			...
			foreach ($ids as $v) {
				...
				update_article_comment_count($comment_info['id_value']);
				//记录日志
				ecjia_merchant::admin_log('评论内容为：'.$comment_info['content'].'，'.__('文章标题是'). $article['title'], 'batch_setup', 'article_comment');
			}
			$pjaxurl = RC_Uri::url('article/merchant/article_comment');
			if (!empty($article_id)) {
				$pjaxurl .= '&id='.$article_id;
