...
;(function(app, $) {
	...
	app.article_info = {
		...
		submit_form : function(formobj) {
			...
			var option = {
				rules : {
					title : { required : true },
					article_cat : { required : true , min : 1},
				},
				messages : {
					title : { required : "请输入文章标题！" },
					article_cat : { min : '请选择文章分类！' },
				},
	...
	app.link_goods = {
		...
		add_link_article : function() {
			...
			.on('click', function() {
				var $this = $(this),
					tmpobj = $( '<li class="ms-elem-selection"><input type="hidden" name="goods_id[]" value="' + $this.attr('data-id') + '" />' + $this.text() + '<span class="edit-list"><i class="fontello-icon-minus-circled ecjiafc-red del"></i></span></li>' );
