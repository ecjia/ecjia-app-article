...
<ul class="nav nav-pills">
	<li class="{if !$smarty.get.type}active{/if}">
		<a class="data-pjax" href="{RC_Uri::url('article/admin/article_comment_list')}
	...
	<li class="{if $smarty.get.type eq 'has_checked'}active{/if}">
		<a class="data-pjax" href='{RC_Uri::url("article/admin/article_comment_list", "type=has_checked
			{if $id}&id={$id}{/if}{if $publishby}&publishby={$publishby}{/if}{if $filter.keywords}&keywords={$filter.keywords}{/if}")}'>
			{t}通过审核{/t}
	...
	<li class="{if $smarty.get.type eq 'wait_check'}active{/if}">	
		<a class="data-pjax" href='{RC_Uri::url("article/admin/article_comment_list", "type=wait_check
			{if $id}&id={$id}{/if}{if $publishby}&publishby={$publishby}{/if}{if $filter.keywords}&keywords={$filter.keywords}{/if}")}'>
			{t}待审核{/t}
	...
	<li class="{if $smarty.get.type eq 'rubbish_comments'}active{/if}">
		<a class="data-pjax" href='{RC_Uri::url("article/admin/article_comment_list", "type=rubbish_comments
			{if $id}&id={$id}{/if}{if $publishby}&publishby={$publishby}{/if}{if $filter.keywords}&keywords={$filter.keywords}{/if}")}'>
			{t}垃圾评论{/t}
	...
	<li class="{if $smarty.get.type eq 'trash'}active{/if}">
		<a class="data-pjax" href='{RC_Uri::url("article/admin/article_comment_list", "type=trash
			{if $id}&id={$id}{/if}{if $publishby}&publishby={$publishby}{/if}{if $filter.keywords}&keywords={$filter.keywords}{/if}")}'>
			{t}回收站{/t}
	...
	<form method="post" action="{$search_action}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}" name="searchForm">
		<div class="btn-group f_l m_r5">
			...
			<ul class="dropdown-menu">
				<li><a class="button_remove" data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/comment_batch" args="sel_action=button_remove&article_id={$id}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要删除的评论{/t}" data-name="id" href="javascript:;"><i class="fontello-icon-trash"></i>{t}删除评论{/t}</a></li>
				<li><a class="button_hide"   data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/comment_batch" args="sel_action=button_hide&article_id={$id}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要移至回收站的评论{/t}" data-name="id" href="javascript:;"><i class="fontello-icon-box"></i>{t}回收站{/t}</a></li>
				<li><a class="button_show"   data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/comment_batch" args="sel_action=button_show&article_id={$id}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要审核通过的评论{/t}" data-name="id" href="javascript:;"><i class="fontello-icon-ok-circled"></i>{t}审核通过{/t}</a></li>
				<li><a class="button_rubbish"data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/comment_batch" args="sel_action=button_rubbish&article_id={$id}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要设为垃圾评论的评论{/t}" data-name="id" href="javascript:;"><i class="fontello-icon-cancel-circled"></i>{t}设为垃圾评论{/t}</a></li>
			</ul>
		</div>
		<div class="choose_list f_r" >
			<input type="text" name="keywords" value="{$smarty.get.keywords}" placeholder="{t}输入用户名,文章标题或评论关键字{/t}"/>
				...
				<tr>
				    ...
				    <td class="hide-edit-area">
						...
						<div class="edit-list">
							...
							<!-- {if ($list.comment_approved eq 'spam') || ($list.comment_approved eq 'trash')} -->
								...
								<!-- {/if}  -->
								<a class="ajaxremove ecjiafc-red" data-toggle="ajaxremove" data-msg="{t}您确定要删除这条评论吗？{/t}" href='{url path="article/admin/comment_remove" args="id={$list.id}&article_id={$list.id_value}"}' title="{t}永久删除{/t}">
									{t}永久删除{/t}
								</a>
							<!-- {/if} -->
						</div>
				    ...
				    <td>
						...
						<!-- {elseif $list.comment_approved eq '0'} -->
								<span style="color:#08c;">{t}待审核{/t}</span>
						<!-- {elseif $list.comment_approved eq 'trash'} -->
								<span class="ecjiafc-red">{t}回收站{/t}</span>
						<!-- {elseif $list.comment_approved eq 'spam'} -->
								<span class="ecjiafc-red">{t}垃圾评论{/t}</span>
						<!-- {/if} -->
					</td>
				</tr>
				<!-- {foreachelse} -->
