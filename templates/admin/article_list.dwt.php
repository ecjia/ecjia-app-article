		...
		<div class="row-fluid  ecjiaf-tac">
			<div>
				<select class="noselect no_search w200" size="15" name="target_cat">
					<option value="0" disabled>{t}全部分类{/t}</option>
			...
			<div>
				<a class="btn btn-gebo m_l5" data-toggle="ecjiabatch" data-name="article_id" data-idClass=".checkbox:checked" data-url="{$form_action}&sel_action=move_to&" data-msg="{t}是否将选中文章转移至分类？{/t}" data-noSelectMsg="{t}请先选中要转移的文章{/t}" href="javascript:;" name="move_cat_ture">{t}开始转移{/t}</a>
			</div>
		</div>
	</div>
</div>
	...
	<form method="post" action="{$search_action}{if $type}&type={$type}{/if}" name="searchForm">
		<div class="btn-group f_l m_r5">
			...
			<ul class="dropdown-menu">
				<li><a class="button_remove" data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/batch" args="sel_action=button_remove{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请先选中要删除的文章{/t}" data-name="article_id" href="javascript:;"><i class="fontello-icon-trash"></i>{t}删除文章{/t}</a></li>
				<li><a class="button_hide"   data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/batch" args="sel_action=button_hide{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要移到回收站的文章{/t}" data-name="article_id" href="javascript:;"><i class="fontello-icon-box"></i>{t}回收站{/t}</a></li>
				<li><a class="button_show"   data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/batch" args="sel_action=button_show{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要通过的文章{/t}" data-name="article_id" href="javascript:;"><i class="fontello-icon-ok-circled"></i>{t}审核通过{/t}</a></li>
				<li><a class="button_rubbish"data-toggle="ecjiabatch" data-idClass=".checkbox:checked" data-url='{url path="article/admin/batch" args="sel_action=button_rubbish{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要这么做吗？{/t}" data-noSelectMsg="{t}请选择要设为垃圾文章的文章{/t}" data-name="article_id" href="javascript:;"><i class="fontello-icon-cancel-circled"></i>{t}设为垃圾文章{/t}</a></li>
		...
		<select class="w220" name="cat_id" id="select-cat">
			<option value="0">{t}全部分类{/t}</option>
			<!-- {foreach from=$cat_select key=key item=val} -->
		...
		<div class="choose_list f_r" >
			<input type="text" name="keywords" value="{$smarty.get.keywords}" placeholder="{if $publishby}{t}请输入商家名称或文章标题{/t}{else}{t}请输入文章名称{/t}{/if}"/>
			<button class="btn search_articles" type="button">{t}搜索{/t}</button>
		</div>
	</form>
</div>
		...
		<table class="table table-striped smpl_tbl table-hide-edit">
			<thead>
				<tr>
				    ...
				    <th class="w250">{t}文章标题{/t}</th>
				    <!-- {if $publishby}  -->
				    <th class="w120">{t}商家名称{/t}</th>
				    <!-- {/if}  -->
				    <th class="w120">{t}文章分类{/t}</th>
				    <th class="w150">{t}添加日期{/t}</th>
			...
			<tbody>
				...
				<tr>
				    ...
				    <td class="hide-edit-area">
				    	<span class="cursor_pointer" data-text="textarea" data-trigger="editable" data-url="{RC_Uri::url('article/admin/edit_title')}" data-name="{$list.cat_id}" data-pk="{$list.article_id}" data-title="{t}编辑文章名称{/t}">{$list.title}</span>
				    	<div class="edit-list">
				    		<!-- {if $list.article_approved eq '1'}  -->
				    			<a href='{RC_Uri::url("article/admin/preview", "id={$list.article_id}{if $publishby}&publishby={$publishby}{/if}")}' target="_blank" title="{t}预览{/t}">{t}预览{/t}</a>&nbsp;|&nbsp;
				    		<!-- {/if}  -->
				    		...
				    		<!-- {if ($list.article_approved eq '0') || ($list.article_approved eq '1')} -->
								...
								<a class="ecjiafc-red toggle_view" href='{url path="article/admin/check" args="id={$list.article_id}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要将该文章设为垃圾文章吗？{/t}" data-val="rubbish_article" data-status="{$list.article_approved}">{t}垃圾文章{/t}</a>&nbsp;|&nbsp;
								<a class="ecjiafc-red toggle_view" href='{url path="article/admin/check" args="id={$list.article_id}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}"}' data-msg="{t}您确定要将该文章移至回收站吗？{/t}" data-val="trashed_article" data-status="{$list.article_approved}">{t}移至回收站{/t}</a>&nbsp;|&nbsp;
								<a class="data-pjax" href='{RC_Uri::url("article/admin/edit", "id={$list.article_id}{if $publishby}&publishby={$publishby}{/if}")}' title="{t}编辑{/t}">{t}编辑{/t}</a>&nbsp;|&nbsp;
								{if $list.article_approved eq 1}
						      		<a class="data-pjax" href='{url path="article/admin/comments" args="id={$list.article_id}{if $publishby}&publishby={$publishby}{/if}"}' title="{t}文章评论{/t}">{t}文章评论{/t}</a>&nbsp;|&nbsp;
						     	{/if}
							<!-- {/if} -->
					      	{if $has_goods}
					      		<a class="data-pjax" href='{url path="article/admin/link_goods" args="id={$list.article_id}{if $publishby}&publishby={$publishby}{/if}"}' title="{t}关联商品{/t}">{t}关联商品{/t}</a> 
					      	{/if}
					     	<!-- {if $list.cat_id > 0} -->
						     	...
						     	<!-- {/if} -->
						     	<a class="ajaxremove ecjiafc-red" data-toggle="ajaxremove" data-msg="{t}您确定要删除文章吗？{/t}" href='{RC_Uri::url("article/admin/remove", "id={$list.article_id}{if $publishby}&publishby={$publishby}{/if}")}' title="{t}永久删除{/t}">{t}永久删除{/t}</a>
						     	<!-- {/if} -->
					     	<!-- {/if} -->
					     </div>
					</td>
					<!-- {if $publishby} --><td>{$list.merchants_name}</td><!-- {/if} -->
				    <td><span><!-- {if $list.cat_id > 0} -->{$list.cat_name|escape:html}<!-- {else} -->{t}保留{/t}<!-- {/if} --></span></td>
				    <td><span>{$list.date}</span><br><span>
						{if $list.article_type eq 'article'} {t}普通{/t}
				...
				<!-- {foreachelse} -->
				   <tr><td class="no-records" colspan="{if $publishby}7{else}6{/if}">{t}没有找到任何记录{/t}</td></tr>
				<!-- {/foreach} -->
            </tbody>
         </table>
    	<!-- {$article_list.page} -->
