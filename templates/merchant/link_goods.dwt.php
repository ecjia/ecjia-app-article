		...
		<div class="panel">
			<div class="panel-body panel-body-small">
				<ul class="nav nav-tabs">
					<li><a class="data-pjax" href='{url path="article/merchant/edit" args="id={$smarty.get.id}"}'>{t}通用信息{/t}</a></li>
					<li class="active"><a href="javascript:;">{t}关联商品{/t}</a></li>
				</ul>
			</div>
								...
								<div class="pull-left">
									<div class="form-group">
										<select class="w130" name="cat_id">
											<option value="0">{t}所有分类{/t}</option>
									...
									<div class="form-group">
										<input class="form-control" type="text" name="keyword" placeholder="{t}商品名称{/t}" />
									</div>
									
									<button type="button" class="btn btn-primary" data-toggle="searchGoods"><i class="fa fa-search"></i> {t}搜索{/t}</button>
									
									<div class="form-group">
										<span class="help-block">{t}搜索要关联的商品，搜到商品会展示在左侧列表框中。点击左侧列表中选项，关联商品即可进入右侧已关联列表。保存后生效。您还可以在右侧编辑关联模式。{/t}</span>
									</div>
								</div>
							</div>
							...
							<div class="row draggable">
								<div class="ms-container " id="ms-custom-navigation">
									...
									<div class="ms-selection">
										<div class="custom-header custom-header-align">{t}关联商品{/t}</div>
