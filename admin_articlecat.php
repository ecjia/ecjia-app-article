...
class admin_articlecat extends ecjia_admin {
	...
	public function update() {
		...
		if ($query) {
			...
			$links[] = array('text' => '返回分类列表', 'href' => RC_Uri::url('article/admin_articlecat/init'));
			ecjia_admin::admin_log($cat_name, 'edit', 'articlecat');
			$this->showmessage(sprintf(RC_Lang::lang('catedit_succed'), $cat_name), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('article/admin_articlecat/edit', array(id => $id))));
		} else {
