...
class admin_articlecat extends ecjia_admin {
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台文章分类页面，系统中所有的文章分类都会显示在此列表中。') . '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：') . '</strong></p>' .
			'<p><a href="https://ecjia.com/wiki/帮助:ECJia智能后台:文章分类" target="_blank">'.__('关于文章分类帮助文档').'</a></p>'
		);
		
		$this->assign('ur_here', __('文章分类'));
		$this->assign('action_link', array('text' => __('添加文章分类'), 'href' => RC_Uri::url('article/admin_articlecat/add')));
...
// 		}
	...
	public function add() {
		...
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台添加文章分类页面，可以在此页面添加文章分类信息。') . '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：') . '</strong></p>' .
			'<p><a href="https://ecjia.com/wiki/帮助:ECJia智能后台:文章分类#.E6.B7.BB.E5.8A.A0.E6.96.87.E7.AB.A0.E5.88.86.E7.B1.BB" target="_blank">'.__('关于添加文章分类帮助文档').'</a></p>'
		);
		
		$this->assign('ur_here', __('添加文章分类'));
		$this->assign('action_link', array('text' => __('文章分类'), 'href' => RC_Uri::url('article/admin_articlecat/init')));
	...
	public function insert() {
        ...
        if ($count > 0) {
			return $this->showmessage(sprintf(__('分类名已经存在'), stripslashes($cat_name)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		$links[] = array('text' => __('继续添加新分类'), 'href' => RC_Uri::url('article/admin_articlecat/add'));
		$links[] = array('text' => __('返回分类列表'), 'href' => RC_Uri::url('article/admin_articlecat/init'));
		return $this->showmessage(__('已成功添加'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('article/admin_articlecat/edit', array('id' => $id))));		
	}
	...
	public function edit() {
		...
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台编辑文章分类页面，可以在此页面编辑相应的文章分类信息。') . '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：') . '</strong></p>' .
			'<p><a href="https://ecjia.com/wiki/帮助:ECJia智能后台:文章分类#.E7.BC.96.E8.BE.91.E6.96.87.E7.AB.A0.E5.88.86.E7.B1.BB" target="_blank">'.__('关于编辑文章分类帮助文档').'</a></p>'
		);
		
		$this->assign('ur_here', __('编辑文章分类'));
		$this->assign('action_link', array('text' => __('文章分类'), 'href' => RC_Uri::url('article/admin_articlecat/init')));
	...
	public function update() {
		...
		if (in_array($parent_id, $catid_array)) {
			return $this->showmessage(sprintf(__('分类名的父分类不能设置成本身或本身的子分类'), stripslashes($cat_name)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		return $this->showmessage(sprintf(__('分类 %s 编辑成功'), $cat_name), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('article/admin_articlecat/edit', array('id' => $id))));
	}
	...
	public function edit_sort_order() {
		...
		if (!is_numeric($order)) {
			...
			return $this->showmessage(__('编辑成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_uri::url('article/admin_articlecat/init')) );
		}
	}
	...
	public function remove() {
		...
		if ($count > 0) {
			return $this->showmessage(__('该分类下还有子分类，请先删除其子分类'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}else {
			/* 非空的分类不允许删除 */
			$query = RC_DB::table('article')->where('cat_id', $id)->count();
			if ($query > 0) {
				return $this->showmessage(sprintf(__('分类下还有文章，不允许删除非空分类')), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			} else {
