		...
		if ($is_only != 0) {
			...
			if(empty($file_name)){
				...
				if (!empty($file)&&((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))) {
					...
					//目前限制只能上传文件
					if(strrpos(RC_Config::load_config('system', 'UPLOAD_FILE_EXT'),$extname)){
// 						$upload = RC_Upload::uploader('file', array('save_path' => 'data/article' . $dir, 'auto_sub_dirs' => true));
						$upload = RC_Upload::uploader('file', array('save_path' => 'data/article', 'auto_sub_dirs' => true));
						$image_info = $upload->upload($file);
						/* 判断是否上传成功 */
						if (!empty($image_info)) {
...
// 						$upload = RC_Upload::uploader('image', array('save_path' => 'data/article' . $dir, 'auto_sub_dirs' => true));
						$upload = RC_Upload::uploader('file', array('save_path' => 'data/article', 'auto_sub_dirs' => true));
						$image_info = $upload->upload($file);
						/* 判断是否上传成功 */
						if (!empty($image_info)) {
...
// 			$open_type = trim($_POST['FCKeditor1']);
// 			$open_type = empty($open_type)? 1 : 2;
			$data = array(
					'title'        => $title,
					'cat_id'  	   => $cat_id,
					'article_type' => intval($_POST['article_type']),
			...
			if($query) {
				...
				$this->showmessage($note, ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array( 'pjaxurl' => RC_Uri::url('article/admin/edit', array(id => $_POST[id]))));
			} else {
	...
	public function insert_link_goods() {
		...
		$this->showmessage('成功修改关联商品', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/admin/link_goods', "id=$article_id")));
	}
...
// 		@unlink($uploads_dir_info['basedir']. $old_url);
		...
		if (! empty($article_ids)) {
			switch ($action) {
				...
				case 'button_remove':
					...
					$this->showmessage(RC_Lang::lang('batch_handle_ok_del'), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/admin/init')));
					break;
				...
				case 'button_hide' :
					...
					$this->showmessage(RC_Lang::lang('batch_handle_ok_hide'), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/admin/init')));
					break;
				...
				case 'button_show' :
					...
					$this->showmessage(RC_Lang::lang('batch_handle_ok_show'), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/admin/init')));
					break;
				...
				case 'move_to' :
					...
					if($target_cat <= 0) {
						$this->showmessage(RC_Lang::lang('no_select_act'), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_ERROR);
					}
					...
					$this->showmessage(RC_Lang::lang('batch_handle_ok_move'), ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('article/admin/init')));
					break;
	...
	private	function get_articleslist() {
		...
		$result = $dbview->where($where)->order(array($filter['sort_by'] => $filter['sort_order']))->limit($page->limit())->select();
		$arr = array();
		if(!empty($result)) {
